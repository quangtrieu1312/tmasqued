...
log "=== 8-core forward-path cap probe (inner3422/tcp/NONE(comb1,rps00), 6 clients, $(date -u '+%Y-%m-%d %H:%M')) ==="
...
$SSH "$GW_SSH" "cp ~/tmasqued/docker-compose.yml.fwdcapbak ~/tmasqued/docker-compose.yml; cd ~/tmasqued; ${GW_SUDO} ethtool -L $GW_IF combined ${GW_RSS_COMBINED} 2>/dev/null; ${GW_SUDO} docker compose up -d >/dev/null 2>&1"
log "=== DONE ==="
